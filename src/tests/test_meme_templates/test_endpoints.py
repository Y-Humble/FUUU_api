...
from apps.meme.templates.messages import TemplateResponseMessage
...
async def test_put_endpoint(
    ...
    for val_id, template in enumerate(template_data, 1):
        ...
        put_response = await async_client.put(
            f"/meme/template/{val_id}", json=changed_template.get_form_data()
        )
...
async def test_delete_endpoint(async_client: AsyncClient):
    ...
    for val in list_response.json():
        delete_response = await async_client.delete(
            f"/meme/template/{val["id"]}"
        )
        assert delete_response.status_code == 200
        assert (
            delete_response.json() == TemplateResponseMessage.DELETED_TEMPLATE
        )
